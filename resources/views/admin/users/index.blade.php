...
<div class="py-6 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 flex items-center justify-between">
            ...
            <a href="{{ route('admin.users.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:outline-none focus:border-indigo-700 active:bg-indigo-800 transition">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="status" id="status" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                            ...
                            <option value="unverified" {{ isset($filters['status']) && $filters['status'] == 'unverified' ? 'selected' : '' }}>Unverified</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex space-x-2">
                            <select name="sort_by" id="sort_by" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                                <option value="created_at" {{ isset($filters['sort_by']) && $filters['sort_by'] == 'created_at' ? 'selected' : '' }}>Registration Date</option>
                                <option value="name" {{ isset($filters['sort_by']) && $filters['sort_by'] == 'name' ? 'selected' : '' }}>Name</option>
                                <option value="email" {{ isset($filters['sort_by']) && $filters['sort_by'] == 'email' ? 'selected' : '' }}>Email</option>
                            </select>
                            <select name="sort_order" id="sort_order" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md">
                                <option value="desc" {{ isset($filters['sort_order']) && $filters['sort_order'] == 'desc' ? 'selected' : '' }}>Desc</option>
                                <option value="asc" {{ isset($filters['sort_order']) && $filters['sort_order'] == 'asc' ? 'selected' : '' }}>Asc</option>
                            </select>
                        </div>
                    </div>
                </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if ($user->email_verified_at)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                            <svg class="h-2 w-2 text-green-400 mr-1" fill="currentColor" viewBox="0 0 8 8">
                                    ...
                                    @else
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                                            <svg class="h-2 w-2 text-yellow-500 mr-1" fill="currentColor" viewBox="0 0 8 8">
                                                <circle cx="4" cy="4" r="3" />
                                            </svg>
                                            Unverified
                                        </span>
